...
By default, Mini-Boss set up with Docker Compose will use Redis as its memory backend.
...
!!! important
    If you have set up Mini-Boss using Docker Compose, then Redis is included, no further
    setup needed.
...
is supported which allows the Mini-Boss process itself to start a Weaviate instance.
...
## 🧠 Memory pre-seeding
Memory pre-seeding allows you to ingest files into memory and pre-seed it before running Mini-Boss.

``` shell
$ python auto-gpt/data_ingestion.py -h 
...
In the example above, the script initializes the memory, ingests all files within the `Mini-Boss/auto-gpt/autogpt/auto_gpt_workspace/DataFolder` directory into memory with an overlap between chunks of 100 and a maximum length of each chunk of 2000.
...
GitHub documentation before running Mini-Boss.

!!! attention
    If you use Redis for memory, make sure to run Mini-Boss with `WIPE_REDIS_ON_START=False`

    For other memory backends, we currently forcefully wipe the memory when starting
    Mini-Boss. To ingest data with those memory backends, you can call the
    `data_ingestion.py` script anytime during an Mini-Boss run.

Memories will be available to the AI immediately as they are ingested, even if ingested
while Mini-Boss is running.
