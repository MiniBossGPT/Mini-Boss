...
def start_buddy(name: str, task: str, prompt: str, model=CFG.fast_llm_model) -> str:
    ...
    key, ack = AGENT_MANAGER.create_buddy(task, first_message, model)
    ...
    buddy_response = AGENT_MANAGER.message_buddy(key, prompt)
...
def message_buddy(key: str, message: str) -> str:
    ...
    if is_valid_int(key):
        buddy_response = AGENT_MANAGER.message_buddy(int(key), message)
...
def list_buddys() -> str:
    ...
    return "List of buddys:\n" + "\n".join(
        [str(x[0]) + ": " + x[1] for x in AGENT_MANAGER.list_buddys()]
...
def delete_buddy(key: str) -> str:
    ...
    """
    result = AGENT_MANAGER.delete_buddy(key)
